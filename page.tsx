...
const INITIAL_PRODUCTS: Product[] = [
  {
    id: "prod-1",
    name: "UltraWide Mechanical Productivity Deck",
    description: "Premium anodized aluminum mechanical layout equipped with hot-swappable tactile switches and dynamic sound dampening profiles for long-form engineering programming.",
    price: 189.99,
  ...
  {
    id: "prod-2",
    name: "Ergonomic High-Back Task Chair Matrix",
    description: "Fully adjustable lumbar support framework combined with highly breathable cooling mesh components configured perfectly to handle long study or coding sessions.",
    price: 349.50,
    image: "https://images.unsplash.com/photo-1505797149-43b0069ec26b?w=600",
    category: "Office",
    stock: 7
  },
  {
    id: "prod-3",
    name: "Dual-Axis Desktop Monitor Mount ARM",
    description: "Heavy-duty counterbalanced mechanical arm designed to seamlessly optimize multi-display vertical alignment orientation layouts on engineering workspaces.",
    price: 79.95,
  ...
  {
    id: "prod-4",
    name: "High-Fidelity Sound Ecosystem Bar",
    description: "Premium spatial acoustic audio controller sitting seamlessly beneath monitors to provide crystal-clear frequency spectrum splits during remote project syncs.",
    price: 120.00,
...
export default function Storefront() {
  ...
  const addToCart = (product: Product) => {
    setCart((prev) => {
      ...
      let updated;
      if (existing) {
        updated = prev.map(item => item.product.id === product.id ? { ...item, quantity: item.quantity + 1 } : item);
      } else {
        updated = [...prev, { product, quantity: 1 }];
      }
      localStorage.setItem('horizon_cart', JSON.stringify(updated));
      return updated;
    });
  };
            ...
            {cart.length === 0 ? (
              ...
              <div className="space-y-4 pt-4">
                ...
                <div className="pt-4 border-t space-y-3">
                  ...
                  {/* Functional Coupon Code Box Verification */}
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button 
                        onClick={() => {
                          ...
                          if (code === 'ECELL20') {
                            alert('Success! 20% E-CELL Technical Discount Applied to Basket!');
                          } else {
                  ...
                  <button
                    onClick={() => alert(`Order of $${cartTotal.toFixed(2)} submitted successfully via local state simulation framework!`)}
